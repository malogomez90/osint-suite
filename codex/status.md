...
- **Last progress:** setup/Telegram baseline now verifies at **73 passed** in `tests/test_setup.py` + `tests/test_telegram_bot.py`; recent work also replaced deprecated `PyPDF2` with `pypdf`, added a root `.gitignore` for local verification artifacts, and stopped tracking Python bytecode under `osint_suite/__pycache__/`. Prior work already covered metadata/version, console scripts, and dev/runtime split.
